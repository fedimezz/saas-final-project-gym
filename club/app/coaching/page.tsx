...
function TeamCoachCard({ coach, isDarkMode, delay }: { coach: RealCoach; isDarkMode: boolean; delay: number }) {
  return (
    <motion.div
      ...
      className={`rounded-2xl overflow-hidden shadow-lg ${isDarkMode ? "bg-neutral-900" : "bg-white"}`}
    >
      <div className="relative h-56">
        {coach.photoUrl ? (
          ...
          <div className={`w-full h-full flex items-center justify-center ${isDarkMode ? "bg-neutral-800" : "bg-gray-100"}`}>
            <Dumbbell size={36} className={isDarkMode ? "text-neutral-600" : "text-gray-300"} />
          </div>
        )}
      </div>
      <div className="p-6">
        <h3 className={`text-xl font-bold mb-1 ${isDarkMode ? "text-white" : "text-gray-900"}`}>{coach.name}</h3>
        ...
        {coach.bio && (
          <p className={`text-sm leading-relaxed ${isDarkMode ? "text-neutral-400" : "text-gray-600"}`}>{coach.bio}</p>
        )}
      </div>
    </motion.div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="p-6">
        ...
        <p className={`text-sm leading-relaxed ${isDarkMode ? "text-neutral-400" : "text-gray-600"}`}>{text}</p>
      </div>
    </motion.div>
  );
}
...
export default function CoachingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setCoaches(Array.isArray(data.coaches) ? data.coaches : []);
      } catch {
        // team section just won't render — the rest of the page still works
      } finally {
  ...
  return (
    <PagePublishGate pageKey="coaching">
          ...
          <div className="relative container mx-auto px-4">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <h1 className={`text-5xl md:text-6xl font-bold mb-5 ${heroImage ? "text-white" : isDarkMode ? "text-white" : "text-gray-900"}`}>
                {t("heroTitle", "Un coaching sur-mesure")}
              </h1>
              <p className={`text-xl max-w-2xl mx-auto leading-relaxed ${heroImage ? "text-white/90" : isDarkMode ? "text-neutral-400" : "text-gray-600"}`}>
                {t("heroSubtitle", "Nos coachs certifiés vous accompagnent vers vos objectifs, à votre rythme, avec un suivi personnalisé.")}
              </p>
            </motion.div>
          </div>
        </section>
        ...
        <section className="py-12">
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {cards.map((c, i) => (
                <CoachCard key={i} {...c} isDarkMode={isDarkMode} delay={i * 0.1} />
              ))}
            </div>
            ...
            {!loadingCoaches && coaches.length > 0 && (
              <div className="mt-16">
                <h2 className={`text-2xl font-bold text-center mb-8 ${isDarkMode ? "text-white" : "text-gray-900"}`}>
                  {t("teamTitle", "Notre équipe de coachs")}
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
                  {coaches.map((coach, i) => (
                    <TeamCoachCard key={coach.id} coach={coach} isDarkMode={isDarkMode} delay={i * 0.1} />
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="text-center mt-12">
              <a
                href="/user/register"
                className="inline-block bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-full font-bold transition transform hover:scale-105 shadow-lg"
              >
                {t("ctaLabel", "Réserver une séance découverte")}
              </a>
            </div>
          </div>
        </section>

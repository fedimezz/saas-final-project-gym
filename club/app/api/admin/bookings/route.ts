...
async function sweepOldCancelledBookings() {
  const cutoff = new Date(Date.now() - CANCELLED_RETENTION_MS);
  await prisma.userSession.deleteMany({
    where: { isCancelled: true, cancelledAt: { lte: cutoff } },
  });
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    await sweepOldCancelledBookings().catch((err) =>
    ...
    return NextResponse.json({
      ...
      coaches: allCoaches.map((s) => s.coach),
    });
  } catch (error) {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { count } = await prisma.userSession.deleteMany({ where: { isCancelled: true } });

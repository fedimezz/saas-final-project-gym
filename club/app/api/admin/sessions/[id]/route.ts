...
export async function DELETE(
  ...
  try {
    ...
    const affectedUserIds = [...new Set(session.userSessions.map((us) => us.userId))];
    const activeUserSessionIds = session.userSessions.map((us) => us.id);
